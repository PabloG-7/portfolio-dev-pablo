  ...
  return (
    ...
    >
      ...
      <div className="container-custom relative z-1"> {/* Mudei para z-1 */}
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 z-1"> {/* Adicionei z-1 aqui */}
          {/* Contact Info */}
          <div className="space-y-6 z-1">
            ...
            <div className={`transition-all duration-1000 ${
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl p-6 border border-amber-200/60 dark:border-amber-500/20 z-1">
            ...
            <div className="space-y-4 z-1">
              {contactInfo.map((info, index) => (
                <a
                  key={info.label}
                  href={info.href}
                  className={`group flex items-center space-x-4 p-4 sm:p-5 rounded-2xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-amber-200/60 dark:border-amber-500/20 transition-all duration-500 hover:shadow-lg hover:scale-[1.02] hover:border-amber-500/30 dark:hover:border-amber-400/30 z-1 ${
                    animatedItems.includes(index + 1) ? 'animate-slide-in-left opacity-100' : 'opacity-0 -translate-x-8'
                  }`}
                  style={{ animationDelay: `${(index + 1) * 150}ms` }}
                >
                  <div className={`p-3 rounded-xl ${info.bgColor} border ${info.borderColor} group-hover:scale-110 transition-transform duration-300 z-1`}>
                  ...
                  <Sparkles className="w-4 h-4 text-amber-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300 animate-sparkle z-1" />
                </a>
              ))}
            </div>

            {/* Social Media */}
            <div className={`pt-6 border-t border-amber-200/60 dark:border-amber-500/20 transition-all duration-1000 z-1 ${
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 z-1">
                {socialLinks.map((social, index) => (
                  <a
                    ...
                    className={`group flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-amber-200/60 dark:border-amber-500/20 transition-all duration-500 hover:shadow-lg hover:scale-105 ${social.hoverColor} z-1 ${
                      animatedItems.includes(index + 4) ? 'animate-scale-in opacity-100' : 'opacity-0 scale-95'
                    }`}
                    style={{ animationDelay: `${(index + 4) * 150}ms` }}
                  >
                    <div className={`p-2 rounded-lg ${social.bgColor} border ${social.borderColor} group-hover:scale-110 transition-transform duration-300 z-1`}>
          ...
          {/* Contact Form */}
          <div className={`transition-all duration-1000 z-1 ${
            animatedItems.includes(7) ? 'animate-slide-in-right opacity-100' : 'opacity-0 translate-x-8'
          }`} style={{ animationDelay: '1050ms' }}>
            <div className="relative bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-amber-200/60 dark:border-amber-500/20 rounded-2xl p-6 sm:p-8 shadow-lg hover:shadow-xl transition-all duration-500 z-1">
              
              {/* Decorative elements */}
              <div className="absolute top-4 right-4 w-2 h-2 bg-amber-500 rounded-full animate-pulse z-1"></div>
              <div className="absolute bottom-4 left-4 w-1.5 h-1.5 bg-orange-500 rounded-full animate-pulse delay-1000 z-1"></div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6 z-1">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <User className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-slate-400 z-1" />
                      <input
                        type="text"
                        name="name"
                        value={formData.name}
                        ...
                        required
                        className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/60 dark:bg-slate-700/60 border border-amber-200/60 dark:border-amber-500/20 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all duration-300 backdrop-blur-sm text-slate-900 dark:text-slate-100 z-1"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-slate-400 z-1" />
                      <input
                        type="email"
                        name="email"
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    required
                    className="w-full px-4 py-3 rounded-xl bg-white/60 dark:bg-slate-700/60 border border-amber-200/60 dark:border-amber-500/20 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all duration-300 backdrop-blur-sm text-slate-900 dark:text-slate-100 z-1"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <MessageCircle className="absolute left-3 top-3 w-4 h-4 text-slate-400 z-1" />
                    <textarea
                      name="message"
                      ...
                      required
                      rows={4}
                      className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/60 dark:bg-slate-700/60 border border-amber-200/60 dark:border-amber-500/20 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all duration-300 resize-none backdrop-blur-sm text-slate-900 dark:text-slate-100 z-1"
              ...
              <div className="mt-4 p-3 bg-green-500/10 border border-green-500/20 rounded-xl text-green-700 dark:text-green-400 text-sm text-center opacity-0 transition-opacity duration-300 z-1">
        ...
        {/* Call to Action */}
        <div 
          className={`text-center mt-12 lg:mt-16 transition-all duration-1000 z-1 ${
            isVisible ? 'animate-fade-in-up opacity-100' : 'opacity-0 translate-y-8'
          }`}
